...
export type MistlCampaignLeadRow = {
...
function normalizeMistlCampaignLeadRows(raw: Record<string, unknown>[]): MistlCampaignLeadRow[] {
  return raw.map((row) => ({
    ...(row as MistlCampaignLeadRow),
...
type LeadsQueryResult = {
  rows: MistlCampaignLeadRow[];
  total: number;
  error: { message: string } | null;
};

async function queryMistlCampaignScoredLeadsPage(
  ...
  return {
    rows: normalizeMistlCampaignLeadRows((leadsData ?? []) as Record<string, unknown>[]),
    total: leadsCount ?? 0,
    error: null,
  };
}

export async function fetchMistlCampaignScoredLeadsPage(
  admin: SupabaseClient,
  campaignId: string,
  offset: number,
  limit: number
): Promise<{ rows: MistlCampaignLeadRow[]; total: number }> {
  const result = await queryMistlCampaignScoredLeadsPage(admin, campaignId, offset, limit);
  if (result.error) throw new Error(result.error.message);
  return { rows: result.rows, total: result.total };
}

/** Paginated fetch — Supabase returns at most 1000 rows per request. */
export async function fetchAllMistlCampaignScoredLeads(
  admin: SupabaseClient,
  campaignId: string
): Promise<{ rows: MistlCampaignLeadRow[]; total: number }> {
  const all: MistlCampaignLeadRow[] = [];
  let offset = 0;
  let total = 0;

  for (;;) {
    const chunk = await queryMistlCampaignScoredLeadsPage(
      admin,
      campaignId,
      offset,
      LEADS_PAGE_SIZE

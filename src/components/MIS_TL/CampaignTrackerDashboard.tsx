...
export default function CampaignTrackerDashboard() {
  ...
  const hideClientName = hasRole("mis_tl");
  ...
  const { items, pagination, response, isLoading, isFetching, error, refetch } =
    usePaginatedListQuery<RevenueReportClientGroup>({
      queryKeyPrefix: ["mistl", "campaign-tracker"],
      url: "/api/mistl/campaign-tracker",
      params: listParams,
      listField: "clients",
    });
  ...
  const exportUrl = (format: "csv" | "excel") =>
    buildListApiUrl("/api/mistl/campaign-tracker/export", {
      ...listParams,
      page: undefined,
      limit: undefined,
      format,
  ...
  const handleStatusChange = async (campaignId: string, status: string) => {
    ...
    try {
      const res = await fetch(`/api/mistl/campaign-tracker/${campaignId}/status`, {
        method: "PATCH",
        credentials: "include",
      ...
      await queryClient.invalidateQueries({ queryKey: ["mistl", "campaign-tracker"] });
    } catch (err) {

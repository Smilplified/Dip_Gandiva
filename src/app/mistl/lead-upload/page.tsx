...
export default function MISTLLeadUploadPage() {
  ...
  const fetchRows = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/mistl/lead-upload/campaigns", { credentials: "include" });
        ...
        {loading ? (
          ...
          <Table
            ...
            onRow={(record) => ({
              onClick: () =>
                router.push(
                  `/mistl/campaigns/${record.campaign_id}?delivery_status=delivered`
                ),

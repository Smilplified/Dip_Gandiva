...
export default function MISTLDashboardPage() {
  ...
  const {
    ...
    error: queryError,
    refetch,
  } = useCachedApiQuery<MistlDashboardResponse>(
    ["mistl", "dashboard"],
    "/api/mistl/dashboard",
    { enabled: listEnabled }
  );
  ...
  useEffect(() => {
    if (queryError) {
      message.error(
        queryError instanceof Error ? queryError.message : "Failed to load MIS TL dashboard"
      );
    }
  }, [queryError]);
  ...
  const handleExport = async (type: string, format: "csv" | "excel") => {
    try {
      const url = `/api/mistl/export?type=${encodeURIComponent(type)}&format=${encodeURIComponent(format)}`;

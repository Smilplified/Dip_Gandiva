...
import {
  fetchAllMistlCampaignScoredLeads,
  fetchMistlCampaignScoredLeadsPage,
  type MistlCampaignLeadRow,
} from "@/lib/mistl/campaign-leads";
...
export async function GET(
  ...
  try {
    ...
    if (filesError) {
      console.error("MISTL campaign files fetch error:", filesError.message);
    }
    ...
    let leadsList: MistlCampaignLeadRow[] = [];
    let leadsTotal = 0;
    let leadsPage = 1;
    let leadsLimit = 10;

    if (exportAll) {
      const all = await fetchAllMistlCampaignScoredLeads(admin, campaignId);
      leadsList = all.rows;
      ...
      const page = await fetchMistlCampaignScoredLeadsPage(
        admin,
        campaignId,
        parsed.offset,
        parsed.limit
    ...
    if (exportAll) {
      void logAudit({
        ...
        description: `Exported ${leadsTotal.toLocaleString()} scored leads (MISTL campaign detail)`,
        targetType: "campaign",
        targetId: campaignId,
        targetLabel: String(camp.name ?? campaignId),
        metadata: {
          row_count: leadsTotal,
          source: "mistl_campaign_detail",
        },
        request,
      });
    }
    ...
    console.error("MISTL campaign detail error:", err);

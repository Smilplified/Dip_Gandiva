...
import {
  buildMistlDashboardAggregates,
  fetchMistlDashboardLeads,
} from "@/lib/mistl/dashboard-stats";
...
export async function GET() {
  try {
    ...
    const canViewMistlDashboard = roleNames.includes("mis_tl") || roleNames.includes("admin");
    if (!canViewMistlDashboard) {
    ...
    const leads = await fetchMistlDashboardLeads(admin, orgId, campaignIds);
    const aggregates = buildMistlDashboardAggregates(leads, campaigns);
    ...
    console.error("MISTL dashboard error:", err);

...
async function assertMistlAccess(supabase: Awaited<ReturnType<typeof createClient>>, userId: string) {
  ...
  const canView = roleNames.includes("mis_tl") || roleNames.includes("admin");
  if (!canView) {
    return {
      error: NextResponse.json(
        { error: "Forbidden: MIS TL or Admin role required" },
        { status: 403 }
      ),
    };
  }
...
export async function GET(request: NextRequest) {
  try {
    ...
    const access = await assertMistlAccess(supabase, user.id);
    ...
    console.error("MISTL campaign tracker error:", err);

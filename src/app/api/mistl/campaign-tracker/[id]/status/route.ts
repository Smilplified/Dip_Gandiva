...
export async function PATCH(
  ...
  try {
    ...
    const canEdit = roleNames.includes("mis_tl") || roleNames.includes("admin");
    if (!canEdit) {
      return NextResponse.json(
        { error: "Forbidden: only MIS TL can update campaign status" },
        { status: 403 }
      );
    }
    ...
    void logAudit({
      ...
      metadata: { previous_status: prev.status, new_status: status, source: "mistl_campaign_tracker" },
      request,
    });

    return NextResponse.json({ campaign: updated });
  } catch (err) {
    console.error("MISTL campaign tracker status update error:", err);

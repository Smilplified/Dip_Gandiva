...
export async function GET() {
  try {
    ...
    const { data: leads } = await admin
      .from("leads")
      .select("id, status, qa_status, delivery_status, lead_tagging, updated_at, created_at")
    ...
    type LeadRow = {
      ...
      delivery_status: string | null; lead_tagging: string | null;
    ...
    return NextResponse.json({
      ...
      deliveredLeads: leadList.filter(isDelivered).length,
      deliveredToday: leadList.filter((l) => {
        const d = new Date(l.updated_at ?? l.created_at);
        return d >= todayStart && isDelivered(l);
      }).length,
      _debug: debug,
    });
  } catch (err) {
